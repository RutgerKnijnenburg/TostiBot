...
private readonly IStatePropertyAccessor<InfoState> _infoStateAccessor;
        ...
        public BasicBot(BotServices services, UserState userState, ConversationState conversationState, ILoggerFactory loggerFactory, IOptions<Storage> storage)
        {
            _storage = storage.Value ?? throw new ArgumentException(nameof(Storage));
        ...
        private async Task UpdateTypeState(RecognizerResult luisResult, ITurnContext turnContext)
        {
            ...
            {
                // Get latest GreetingState
                var typeState = await _getTypesStateAccessor.GetAsync(turnContext, () => new TypeState());
                var entities = luisResult.Entities;
                ...
                foreach (var name in userNameEntities)
                {
                    ...
                    if (entities[name] != null)
                    {
                        // Capitalize and set new user name.
                        var newName = (string)entities[name][0];
                        typeState.CurrentType = char.ToUpper(newName[0]) + newName.Substring(1);
                        break;
                    }
                }

                // Set the new values into state.
                await _getTypesStateAccessor.SetAsync(turnContext, typeState);
            }
        }
